...
public class Facul {
    ...
    public void cadastrarCurso() {
        curso[getIndexCurso()].preencher();
        curso[getIndexCurso()].setIndex(getIndexCurso());
    ...
    public void cadastrarAluno() {
        ...
        aluno[getIndexAluno()].preencher();
        aluno[getIndexAluno()].setIndex(getIndexAluno());
        matricula[indexMatricula].setIndex(indexMatricula);
        System.out.println("Deseja se cadastrar em qual curso?");
        do {
            System.out.println("0 - Cadastrar Sem Curso");
            for (int i = 0; i < getIndexCurso(); i++) {
                System.out.print((i + 1) + " - Curso: " + curso[i].getNome());
                System.out.println(" - CH: " + curso[i].getCargaHoraria());
            }
            System.out.println("Digite uma opção: ");
            op = getScan().nextInt();
            if (op > 0 && op <= (getIndexCurso() + 1)) {
                aluno[getIndexAluno()].setCurso(curso[op - 1].getNome());
                opcaoValida = true;
            } else {
        ...
        matricula[getIndexMatricula()].preencher();
        indexAluno++;
        indexMatricula++;
    ...
    public int buscarIndexCurso(String curso) {
        for (int i = 0; i <= getIndexCurso(); i++) {
            if (curso.equals(this.curso[i].getNome())) {
                return this.curso[i].getIndex();
            }
        }
        return -1;
    }
    ...
    public void imprimir() {
        for (int i = 0; i < getIndexAluno(); i++) {
            System.out.println("-----------------------");
            aluno[i].imprimir();
            if (buscarIndexCurso(aluno[i].getCurso()) == -1) {
                System.out.println("Não cadastrado em curso algum.");
            } else {
                getCurso()[buscarIndexCurso(aluno[i].getCurso())].imprimir();
